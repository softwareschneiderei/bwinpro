 ...
 * @author jhansen
 */
public class TeThinFromBelow implements TreatmentElement {

    private final String name = "thinningfrombelow";
    private final String label = "te_thbelow";
    private final String desc = "te_thbelow_info";

    private double maxOut;
    private double thinningFactor;
    private double maxBgradAbsenkung;

    public TeThinFromBelow(double maxOut, double thinningFactor, double maxBgradAbsenkung) {
        init(maxOut, thinningFactor, maxBgradAbsenkung);
    }

    public TeThinFromBelow() {
        this(50, 1.0, 0.3);
    }
...
// berechnen was rausgenommen werden darf                
                ...
                for (int j = 0; j < st.ntrees; j++) {
                    if (st.tr[j].out < 0 && st.tr[j].d >= 7.0) {
                        csagesamt += Math.PI * Math.pow((st.tr[j].cw / 200.0), 2.0) * st.tr[j].fac;
                    }
                }
                ...
                //Abgleich
                for (int iik = 0; iik < st.nspecies; iik++) {
                    baToTakeSp[iik] = baToTakeSp[iik] * baToTakeOut / summe;
                }
                    ...
                    if (maxbaOutsp <= 0.0) {
                        ...
                        do {
                            ...
                            if (merk == -9) {
                                ...
                                maxbaOutsp = maxbaOutsp - (st.tr[merk].fac * Math.PI * Math.pow(st.tr[merk].d / 200.0, 2.0)) / st.size;
                                if (maxbaOutsp <= 0.0) {
                                    doNotEndThinning = false;
                                }
                            }
