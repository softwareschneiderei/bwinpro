...
public class PackageInfo {
    ...
    public PackageInfo() {
        ...
        int counter = 0;
        if (pkgInfo(classLoader, "javax.vecmath", "Point3d")) {
            counter++;
        }
        if (pkgInfo(classLoader, "javax.media.j3d", "SceneGraphObject")) {
            counter++;
        }
        if (pkgInfo(classLoader, "com.sun.j3d.utils.universe", "SimpleUniverse")) {
            counter++;
        }
        if (counter == 3) {
            j3dinstalled = true;
