...
//            }
            ...
            try(Connection db = conn.openDBConnection(database, "", "")) {
                System.out.println("Updated szenarioart with type " + makeThinningTypeDynamic(db, "sts", "10.0/sts/22.0;22.0/sts/28.0;28.0/sts/100.0"));
                System.out.println("Updated szenarioart with type " + makeThinningTypeDynamic(db, "tfb", "10.0/tfb/22.0;22.0/tfb/28.0;28.0/tfb/100.0"));
                db.commit();
            }
        }
    }
    ...
    private static int makeThinningTypeDynamic(final Connection db, String src, String dest) throws SQLException {
        try (PreparedStatement st = db.prepareStatement("update SzenarioArt set"
                    + " ThinningType = ? where ThinningType = ?")) {
            st.setString(1, dest);
            st.setString(2, src);
            return st.executeUpdate();
        }
