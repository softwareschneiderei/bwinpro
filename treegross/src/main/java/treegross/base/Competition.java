...
public class Competition implements PlugInCompetition {
    ...
    public void replaceC66xyAndC66cxy(Tree t, double influenceZoneRadius) {
        ...
        for (int k = 0; k < t.st.ntrees; k++) {
            ...
            if (e < influenceZoneRadius + t.st.tr[j].cw / 2.0 && t.st.tr[j].h > h66 && t.st.tr[j].out < 0) {
                ...
                if (overlap > 0) {
                    percOverlapInStand = getPercOverlapInStand(influenceZoneRadius, t.x, t.y, cri, t.st.tr[j].x, t.st.tr[j].y, t.st);
                    t.c66cxy += t.st.tr[j].fac * (overlap * percOverlapInStand);
                }
            }
        }
     ...
     * @return are of overlap 
     */
    protected static double overlap(double r1, double r2, double e) {
     ...
     */
    protected double getPercCircleInStand(double radius, double x, double y, Stand st) {
     ...
     */
    protected double getPercOverlapInStand(double radius_cz, double x_cz, double y_cz,
