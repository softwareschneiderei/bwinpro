/* http://www.nw-fva.de
   Version 2013-01-13

   (c) 2002 Juergen Nagel, Northwest German Forest Research Station, 
       Grätzelstr.2, 37079 Göttingen, Germany
       E-Mail: Juergen.Nagel@nw-fva.de
 
This program is free software; you can redistribute it and/or
modify it under the terms of the GNU General Public License
as published by the Free Software Foundation.

This program is distributed in the hope that it will be useful,
but WITHOUT  WARRANTY; without even the implied warranty of
MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
GNU General Public License for more details.
 */
package nwfva.biomass;

/**
 * Hilfsklasse zur Übergabe der Biomassefunktionen und Nährstoffkonzentrationswerte
 * @author J. Nagel
 */
public class BiomassSetting {
    
    int code=0;
    String specieslist="";
    String totalBM="";
    String stemBMkg="";
    String barkBMkg="";
    String branchBMkg="";
    String reisigBMkg="";
    String leafBM="";
    String woodDensity="";
    String woodFacC="";
    String woodFacN="";
    String woodFacS="";
    String woodFacP="";
    String woodFacK="";
    String woodFacCa="";
    String woodFacMg="";
    String woodFacMn="";
    String woodFacFe="";
    String woodFacBOup="";
    String woodFacBNH4="";
    String woodFacBNO3="";
    String barkFacC="";
    String barkFacN="";
    String barkFacS="";
    String barkFacP="";
    String barkFacK="";
    String barkFacCa="";
    String barkFacMg="";
    String barkFacMn="";
    String barkFacFe="";
    String barkFacBOup="";
    String barkFacBNH4="";
    String barkFacBNO3="";
    String branchFacC="";
    String branchFacN="";
    String branchFacS="";
    String branchFacP="";
    String branchFacK="";
    String branchFacCa="";
    String branchFacMg="";
    String branchFacMn="";
    String branchFacFe="";
    String branchFacBOup="";
    String branchFacBNH4="";
    String branchFacBNO3="";
    String reisigFacC="";
    String reisigFacN="";
    String reisigFacS="";
    String reisigFacP="";
    String reisigFacK="";
    String reisigFacCa="";
    String reisigFacMg="";
    String reisigFacMn="";
    String reisigFacFe="";
    String reisigFacBOup="";
    String reisigFacBNH4="";
    String reisigFacBNO3="";
    String leafFacC="";
    String leafFacN="";
    String leafFacS="";
    String leafFacP="";
    String leafFacK="";
    String leafFacCa="";
    String leafFacMg="";
    String leafFacMn="";
    String leafFacFe="";
    String leafFacBOup="";
    String leafFacBNH4="";
    String leafFacBNO3="";    
    String taperFuctionClass="";
    int taperFunctionNumber=1;
    String stemVolumeFunction="";

    /** Creates a new instance of LoggingSortiment */
    public BiomassSetting() {
    }
   
}
